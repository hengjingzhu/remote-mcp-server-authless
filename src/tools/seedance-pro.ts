...
export function registerSeedanceProTool(server: McpServer, getBearerToken: () => Promise<string | null>) {
    server.tool(
        ...
        "Generates high-quality videos using ByteDance SeedanceV1-Pro model. Supports both text-to-video (T2V) and image-to-video (I2V) generation. This professional video generation model creates 5s or 10s videos at 480p resolution with advanced motion dynamics and multi-shot support. Perfect for creative content, prototyping, and commercial video production.",
        {
            ...
            duration: z.number().int().min(5).max(10).optional().default(5).describe("Video duration in seconds. Supported values: 5 or 10 seconds. Default is 5 seconds(recommand) for faster generation and smaller file sizes. 10 seconds creates longer videos but takes more processing time unless the user required."),
            resolution: z.enum(["480p", "720p"]).optional().default("480p").describe("Video resolution quality. '480p' (recommend) for faster generation and smaller files, '720p'(not recommend) for balanced quality and speed which not recommand unless user required"),
